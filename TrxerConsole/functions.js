...
var myColor = ["#76FF9F", "#ff5050", "#FBE87E"];//green,red,yellow
...
function CreateHorizontalBars(id, totalPass, totalFailed, totalWarn) {
    ...
    var canvas;
    var ctx;
    var Passed = totalPass;
    var Failed = totalFailed;
    var Warning = totalWarn;
    //alert(Passed);
    var total = Passed + Failed + Warning;
    var Total = Passed + Failed + Warning;
   ...
   /* for (var i = 0 ; i < myArray.length; i++) {
    ...
    ctx.moveTo(100, 0);
    ctx.fillStyle = myColor[i];
    ctx.fillRect(currX+width+1, (canvas.height - myArray[i]) + 25, width, h);
    ctx.fillStyle = myColor[i+1];
    ctx.fillRect(currX+width + 1, (canvas.height - myArray[i + 1]) + 25, width, h);
    ctx.fillRect(currX, (canvas.height - myArray[i + 2]) + 25, width, h);
    */
...
function CalculateTotalPrecents() {
    ...
    //Only percentage
    myParsedData.push("" + Math.round(passedPrec).toFixed(0) + "%");
    myParsedData.push("" + Math.round(failedPrec).toFixed(0) + "%");
...
function CalculateTestsStatuses(testContaineId, canvasId) {
    ...
    document.getElementById(testContaineId + "Passed").innerHTML = totalPassed;
    document.getElementById(testContaineId + "Failed").innerHTML = totalFailed;
    document.getElementById(testContaineId + "Warning").innerHTML = totalInconclusive;
